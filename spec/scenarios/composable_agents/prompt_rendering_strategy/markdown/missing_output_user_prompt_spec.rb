describe ComposableAgents::PromptRenderingStrategy::Markdown, '#missing_output_user_prompt' do
  it 'prompts for missing artifacts as bullet points' do
    expect(
      agent_for_markdown.missing_output_user_prompt(
  ...
  it 'handles single missing artifact correctly' do
    expect(agent_for_markdown.missing_output_user_prompt(result: 'Calculation result')).to eq <<~EO_PROMPT
      Some artifacts are missing:
